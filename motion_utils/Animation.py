...
import numpy as np
# numpy.core.umath_tests removed in numpy 2.x; matrix_multiply = np.matmul
import numpy as _np_compat
class _ut:
    @staticmethod
    def matrix_multiply(a, b):
        return _np_compat.matmul(a, b)
ut = _ut()
...
# Attach module-level functions to the Animation class so that
# `Animation.positions_global(anim)` etc. work as expected by callers.
Animation.positions_global = staticmethod(positions_global)
