...
def load(filename, start=None, end=None, order=None, world=False):
    ...
    channel_counts = []  # per-joint channel count (in joint order)
    ...
    for line in f:
        ...
        if dmatch:
            ...
            # --- Mixed-channel BVH support ---
            # If all joints have the same channel count (uniform), use the fast
            # legacy path. Otherwise, parse per-joint using channel_counts.
            all_same = len(set(channel_counts)) == 1
            if all_same and channel_counts:
                channels = channel_counts[0]
                if channels == 3:
                    # Root position in first 3 values, then N*3 rotations
                ...
                # Mixed channels: parse each joint individually
                offset = 0
                for j, nchan in enumerate(channel_counts):
                    vals = data_block[offset:offset + nchan]
                    offset += nchan
                    if nchan == 6:
                        positions[fi, j] = vals[0:3]
                        rotations[fi, j] = vals[3:6]
                    elif nchan == 3:
                        rotations[fi, j] = vals[0:3]
                    # channels == 9 mixed case not expected from Blender; skip
